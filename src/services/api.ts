...
export async function getPrompts(categoryId?: string) {
  try {
    ...
    // 返回mock数据，根据categoryId过滤
    if (categoryId && categoryId !== 'all') {
      return mockPrompts.filter(prompt => prompt.category_id === categoryId);
    }
    return mockPrompts;
  }
}
...
export async function getPromptById(id: string) {
  try {
    ...
    console.warn('API request failed, using mock data:', error);
    // 返回mock数据
    return mockPrompts.find(prompt => prompt.id === id) || mockPrompts[0];
  }
}
...
export async function getComments(promptId: string) {
  try {
    ...
    console.warn('API request failed, using mock data:', error);
    // 返回mock数据，根据promptId过滤
    return mockComments.filter(comment => comment.prompt_id === promptId);
  }
}
