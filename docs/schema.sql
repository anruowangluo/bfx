...
BEGIN
  ...
  RETURN (SELECT likes_count FROM prompts WHERE id = prompt_id_param);
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  RETURN (SELECT saves_count FROM prompts WHERE id = prompt_id_param);
END;
$$ LANGUAGE plpgsql;
...
INSERT INTO prompts (id, title, description, content, author_id, author_name, author_avatar, category_id, category_name, category_color, tags, likes_count, saves_count, image) VALUES
...
INSERT INTO comments (id, prompt_id, author_id, author_name, author_avatar, content) VALUES
